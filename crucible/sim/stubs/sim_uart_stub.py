...
#     to the log file; also detect the session-end sentinel and write the
#     sentinel file.
#   - Pass all other register reads/writes silently (return 0 on read).
#
# Configuration is communicated via well-known files:
#   ~/.crucible_uart_log_path.txt      -- absolute path to uart log file
#   ~/.crucible_uart_sentinel_path.txt -- absolute path to SESSION_END sentinel
#   ~/.crucible_uart_sentinel_str.txt  -- the session-end marker string
#                                         (default: "SESSION_END" if missing)
#
...
_CFG_LOG   = os.path.expanduser("~/.crucible_uart_log_path.txt")
_CFG_SENT  = os.path.expanduser("~/.crucible_uart_sentinel_path.txt")
_CFG_SSTR  = os.path.expanduser("~/.crucible_uart_sentinel_str.txt")
...
def _session_end_str():
    return _read_cfg(_CFG_SSTR, "SESSION_END")
...
if request.IsInit:
    ...
    if log_path:
        ...
        try:
            ...
            pass
    self.NoisyLog("sim_uart_stub: initialized, log=%s, sentinel=%s" % (
        log_path, _session_end_str()))
    ...
    if off == TXD_PTR:
        ...
        if count > 0 and ptr != 0:
            ...
            for i in range(count):
                try:
                    ...
                    if not _err_logged:
                        _err_logged = True
                        try:
                            ef = open(os.path.expanduser("~/.crucible_readbyte_err.txt"), "w")
            ...
            # Detect session-end sentinel across line buffer
            _line_buf = _line_buf + text
            if _session_end_str() in _line_buf:
