...
Parses structured log lines that firmware emits over UART into typed event
objects (StepEvent, SnapshotEvent, SessionEndEvent).

Firmware log formats:
...
def parse_line(
    ...
    """Parse a single UART log line into a typed event, or None."""
    line = line.strip()
...
def parse_uart_log(text: str) -> tuple[
    ...
    """Parse a full UART log string into (steps, snapshots, session_ends)."""
...
def parse_binary_export_log(text: str) -> list[SnapshotEvent]:
    ...
    Hex-decodes those lines, prepends the GA1T magic header, and calls
    parse_binary_snapshots() so the same unpack path used by the real BLE
    host tool is exercised.
...
def parse_binary_snapshots(data: bytes) -> list[SnapshotEvent]:
    """Unpack a binary snapshot blob (GA1T magic + count + N × 20-byte structs).
